...
module.exports = function (s, conf) {
  ...
  function nextBuyForQuote(s, quote) {
    if (s.next_buy_price)
      ...
      return n(quote.ask).subtract(n(quote.ask).multiply(s.options.markdown_buy_pct / 100)).format(s.product.increment, Math.floor)
  }
  function nextSellForQuote(s, quote) {
    if (s.next_sell_price)
      return n(s.next_sell_price).format(s.product.increment, Math.ceil)
    else
      return n(quote.bid).add(n(quote.bid).multiply(s.options.markup_sell_pct / 100)).format(s.product.increment, Math.ceil)
  }
